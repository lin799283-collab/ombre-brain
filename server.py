...
# --- Compatibility patch: bypass strict Content-Type validation added in mcp 1.27.1 ---
# Kelivo and some MCP clients omit Content-Type: application/json → 400/415 without this.
try:
    import mcp.server.transport_security as _ts
    _ts.TransportSecurityMiddleware._validate_content_type = lambda self, ct: True
except Exception:
    pass
try:
...
if __name__ == "__main__":
    ...
    if transport in ("sse", "streamable-http"):
        ...
        # --- ASGI wrapper: inject Content-Type header on POST requests missing it ---
        # Belt-and-suspenders alongside the monkey-patch above.
        class _ContentTypePatch:
        ...
        # --- GET /mcp SSE keepalive: prevents Cloudflare/proxy idle timeouts ---
        from starlette.applications import Starlette
        from starlette.routing import Route, Mount
        from starlette.requests import Request
        from starlette.responses import StreamingResponse

        async def _mcp_keepalive(request: Request):
            async def _gen():
                yield ": connected\n\n"
                while True:
        ...
        root_app = Starlette(routes=[
            Route("/mcp", _mcp_keepalive, methods=["GET"]),
            Mount("/", app=_ContentTypePatch(_app)),
        ])

        uvicorn.run(root_app, host="0.0.0.0", port=OMBRE_PORT)
    else:
